...
const INITIAL_GEMS = [
  ...
  // Additional Mock Data to ensure variety
  {
    id: 26,
...
const getRecommendations = (currentGem, allGems) => {
    // [수정] 샘플 데이터(ID가 숫자)는 제외하고, 실제 사용자 데이터(ID가 문자열)만 추천
    const realUserGems = allGems.filter(g => typeof g.id === 'string' && g.id !== currentGem.id);

    if (realUserGems.length === 0) return []; // 사용자 데이터가 없으면 빈 배열 반환 (추천 안 뜸)
    ...
    // 로직 2: 같은 카테고리 1등 (이미 선택된 것 제외하고 1개)
...
const GemDetailModal = ({ gem, allGems, onClose, onSwitchGem, onLike, onReport, onDelete }) => {
  ...
  useEffect(() => {
      ...
      // document.querySelector('.gem-detail-modal-content')?.scrollTo(0, 0); // Removed problematic line
  }, [gem]);
...
// [수정] GemRegisterModal: Placeholder 문구 변경 (요청사항 반영)
...
export default function App() {
  ...
  const [realGems, setRealGems] = useState([]); // Store Firestore data
  ...
  const [isFilterOpen, setIsFilterOpen] = useState(true); // [변경] 기본값을 true로 설정
  ...
  useEffect(() => {
      if (!db || !user) return; // [수정] Wait for user auth
      // [수정] Correct Path usage
      ...
      return () => unsubscribe();
  }, [user]); // [수정] Depend on user

  // Merge Real + Mock Data with Duplicate Logic
  useEffect(() => {
      // 1. Prepare Mock Data (25x Duplication + Randomization)
      let hugeGems = [];
      const prefixes = ["🔥Hot", "✨New", "👍Best", "⭐Top", "⚡Fast", "💡Tip"];
      
      for (let i = 0; i < 25; i++) {
          const batch = INITIAL_GEMS.map(g => ({
              ...g,
              id: g.id + (i * 10000), // Ensure unique IDs for mock items
              title: i > 0 && Math.random() > 0.7 ? `[${prefixes[Math.floor(Math.random() * prefixes.length)]}] ${g.title}` : g.title, // Add random prefix
              xp: Math.floor(g.xp * (0.5 + Math.random() * 1.5)), // Randomize XP (0.5x ~ 2.0x)
              views: Math.floor(g.views * (0.5 + Math.random() * 1.5)) // Randomize Views
          }));
          hugeGems = [...hugeGems, ...batch];
      }
      
      const shuffledMock = hugeGems.sort(() => 0.5 - Math.random());

      // 2. Combine: Real Gems (First) + Mock Gems (After)
      setGems([...realGems, ...shuffledMock]);

  }, [realGems]); // Re-run when real data changes

  const handleRefresh = async () => {
      setIsRefreshing(true);
      await new Promise(resolve => setTimeout(resolve, 800));
      
      // Refresh Logic: Re-shuffle Mock Data + Keep Real Data
      let hugeGems = [];
      const prefixes = ["🔥Hot", "✨New", "👍Best", "⭐Top", "⚡Fast", "💡Tip"];
      ...
      for (let i = 0; i < 25; i++) {
          const batch = INITIAL_GEMS.map(g => ({
              ...g,
              id: g.id + (i * 10000) + Math.floor(Math.random()*100), 
              title: i > 0 && Math.random() > 0.7 ? `[${prefixes[Math.floor(Math.random() * prefixes.length)]}] ${g.title}` : g.title,
              xp: Math.floor(g.xp * (0.5 + Math.random() * 1.5)), 
              views: Math.floor(g.views * (0.5 + Math.random() * 1.5))
          }));
          hugeGems = [...hugeGems, ...batch];
      }
      const shuffledMock = hugeGems.sort(() => 0.5 - Math.random());
  ...
  // [Updated] Handle XP (Kiss) Increase
  const handleXPIncrease = async (id, amount) => { 
      ...
      // 2. If it's a real gem (string ID), update Firestore
      // [수정] Correct Path usage
  ...
  const handleViewIncrease = async (id) => {
      ...
      // 2. If it's a real gem, update Firestore
      // [수정] Correct Path usage
      if (typeof id === 'string' && db && user) {
          try {
              const gemRef = doc(db, 'artifacts', appId, 'public', 'data', COLLECTION_NAME, id);
  ...
  return (
    ...
    >
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-2 h-12 flex justify-between items-center">
            {isSearchOpen ? (
                ...
                <>
                    {/* [수정] 3형제 파란 네모 로고 (SVG) 적용 + THE FIRST 태그 제거(Footer로 통합) */}
      ...
      {/* [신규] 카테고리별 1위 배너 (스타 만들기 - 현재 필터링된 결과 중 1위) */}
      {topStar && (
